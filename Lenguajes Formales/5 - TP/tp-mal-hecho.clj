...
(defn evaluar [sentencia amb]
  (if (or (contains? (set sentencia) nil) (and (palabra-reservada? (first sentencia)) (= (second sentencia) '=)))
      ...
      (case (first sentencia)
        ...
        LIST  (let [cuerpo (first amb)]
                  [(map println (map #(apply str (first %) " " (interpose " " (fnext %))) cuerpo)) amb]
              )
        RUN (cond
...
(defn palabra-reservada? [x]
  (let [palabras-reservadas (set '(AND, ASC, ATN, CHR$, CLEAR, DATA, END,
          FOR, GOSUB, GOTO, IF, INPUT, INT,  LEN, LET, LIST, LOAD, MID$,                                                  ;SUPONGO QUE SON SOLO LAS QUE APARECEN EN EL ENUNCIADO DEL TP  
          NEW, NEXT, NORMAL, NOT, ON, OR, PRINT, READ, REM, RESTORE,
          RETURN, RUN, SAVE, SIN, STEP, STORE, STR$, THEN, TO))]                                                          ; AND ES OPERADOR Y PALABRA RESERVADA                                                                                                                   
...
(defn operador? [x]
  (let [operadores (set (list (symbol "^") '= '- '+ '/ '*                                        
        'AND 'OR 'NOT                                                                             ;PREGUNTAR SI AND, OR, ETC PUEDEN SER OPERADORES Y PALABRAS RESERVADAS
        '< '> '<= '=< '>= '=> '<> '><))]                                               
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn checkear-coma [n]
  (contains? (set n) (symbol ","))
)
...
(defn expandir [lista parametro]
  (let [lista-sin-coma (quitar-coma lista)]
    (cond
      ...
      (and (= (first lista) parametro) (nil? (next lista)))       lista
      :else                                                       (list lista)
    ) 
  )
)
...
(defn expandir-nexts [n]
  (cond
    (checkear-lista-de-listas n)  (apply concat (map expandir-nexts n))
...
; false
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn comienza-con-letra? [x]
  (contains? (set "ABCDEFGHIJKLMNOPQRSTUVWXYZ") (first (str x)))
)
...
(defn variable-float? [x]
  (and (comienza-con-letra? x) (not (variable-integer? x)) (not (variable-string? x)) (not (simbolo-o-palabra-reservada? x))) 
)
...
(defn variable-string? [x]
  (and (some? (re-seq #"\$" (str x))) (not (simbolo-o-palabra-reservada? x)))
)
...
(defn construir-puntero [nuevo-puntero]
  (vec (list (first nuevo-puntero) (dec (last nuevo-puntero))))
)
...
(defn continuar-linea [amb]
  (cond
    (empty? (first (nnext amb)))        (vec (list (dar-error (buscar-mensaje 22) (first (next amb))) amb))
    :else                               (vec (list :omitir-restante (actualizar-puntero amb)))
  )
)                                                                                     
...
(defn operar ([operador operando]
                (cond
                  (= '-u operador)  (- operando)
                  (= 'SIN operador) (Math/sin operando)
                  (= 'ATN operador) (Math/atan operando)
                  (= 'INT operador) (int operando)
                  (= 'LEN operador) (count operando)
                  :else             "Operacion unaria no definida."
                )
              )
              ([operador operando1 operando2]
                (cond
                  ...
                  (= '* operador)             (* operando1 operando2)
                  (= '/ operador)             (/ operando1 operando2)
                  (= (symbol "^") operador)   (pot operando1 operando2)
                  :else                       "Operacion binaria no definida"
                )
              )
)

(defn checkear-variable [elemento set-de-variables]
  (cond
    (contains? set-de-variables elemento)   (get set-de-variables elemento)
    :else                                   elemento
  )
)
...
(defn ejecutar-asignacion [sentencia amb]
  (let [variable (first sentencia),
        set-de-variables (last amb),
        asignacion (nnext sentencia)]                                          ; PUEDE LLEGAR CUALQUIER OPERACION ASIGNABLE
    (cond
      (and (variable-string? variable) (= (count sentencia) 5))         (conj (vec (butlast amb)) (merge set-de-variables (array-map variable (apply str (get set-de-variables variable) (last sentencia)))))
      (= (count sentencia) 4)                                           (conj (vec (butlast amb)) (merge set-de-variables (array-map variable (operar (first asignacion) (checkear-variable (last asignacion) set-de-variables))))) 
      (= (count sentencia) 5)                                           (conj (vec (butlast amb)) (merge set-de-variables (array-map variable (operar (second asignacion) (get set-de-variables (first asignacion)) (checkear-variable (last asignacion) set-de-variables)))))                                       
      :else                                                             (conj (vec (butlast amb)) (merge set-de-variables (array-map variable (last sentencia))))
    )
  )
)
...
; (5 + 0 / 2 * 0)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn reemplazar-puntos [expr]
  (map #(if (= '. %) 0 %))
)


(defn preprocesar-expresion [expr amb]
  (map #(if (contains? (last amb) %) (get (last amb) %) (if (variable-string? %) "" (if (or (variable-integer? %) (variable-float? %)) 0 (if (= '. %) 0 %)))) expr) 
)

;(cond
;    (list? expr)                                              (map preprocesar-expresion expr amb)
;    (contains? (last amb) expr)                               (get (last amb) expr)                              ;NO FUNCIONA ESTA LINEA, DICE QUE NO TOMA CONTAINS?, NI IDEA
...
; (MID3$ ( 1 , -u 2 + K , 3 ))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn quitar-todo-menos-coma [n]
  (remove #(not= (symbol ",") %) n)
)


(defn reemplazar-operador [operador resto]
  (cond
    (= '+ operador)        nil
    (= '- operador)        '-u
    (= 'MID$ operador)     (if (= 2 (count (quitar-todo-menos-coma (next resto)))) 'MID3$ operador)
    :else                  "Operador no desambiguable"
  )
)



(defn checkear-puntuacion [x]
  (let [puntuacion (set (list (symbol "(") (symbol ")") (symbol ",")))]
    (contains? puntuacion x)
  )
)

;(defn anular-parentesis [expr]
;  (remove nil? (map #(if (parentesis? %) nil %) expr))
;)             


(defn desambiguar ([expr]
                  (cond
                    (operador? (first expr))  (remove nil? (cons (reemplazar-operador (first expr) (next expr)) (desambiguar (next expr) false)))
                    (= 'MID$ (first expr))    (cons (reemplazar-operador (first expr) (next expr)) (desambiguar (next expr) false))
                    :else                                                  expr
                  ))
                  ([expr check-flag]
                    (cond
                      (false? check-flag)   (if (checkear-puntuacion (first (next expr))) (cons (first expr) (cons (second expr) (desambiguar (nnext expr) true))) (cons (first expr) (desambiguar (next expr) false)))   
                      :else                 (desambiguar expr)
                    )
                  )
...
(defn precedencia [token]
  (cond
    ...
    :else (case token
            ...
            (+ -)                     5
            (* /)                     6
            -u                        7                           ; SI HAY PROBLEMA CON LOS PARENTESIS PONERLES DE PRECEDENCIA NIL                   
                                      9)
  )
)
...
; 3
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(defn aridad [token]                                                     ;Se le asigna aridad distinta de 0 a funciones y operadores
  (cond
    (= (symbol "^") token)                                       2
    :else (case token
                (NOT ATN INT SIN LEN ASC CHR$ STR$)              1
                (+ - = <> >< < > <= =< >= => * / OR AND MID$)    2
                MID3$                                            3
                                                                 0)
  )
